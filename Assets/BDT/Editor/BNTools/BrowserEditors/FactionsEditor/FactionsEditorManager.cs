...
public class FactionsEditorManager : EditorWindow
{
    ...
    private void ModifySelectedAssets()
    {
        ...
        {
            EditorGUILayout.HelpBox("Zero Faction Selected, " + "\n" + "select at least one Faction before edit it.", MessageType.Error);
        ...
        else
        {
            ...
            EditorGUILayout.HelpBox("Modify selected Faction Assets.", MessageType.Info);
            ...
            if (A_index == 2)
            {
                ...
                if (GUILayout.Button("Remove Selected"))
                {
                    ...
                    foreach (var dic in modifyDic)
                    {
                        ...
                        currMod.modFilesData.factionsData.factions.Remove(dic);
    ...
    private void WindowModeEdit(ModuleReceiver currMod)
    {
        ...
        if (EditorGUI.EndChangeCheck())
        {
            ...
            for (int i2 = 0; i2 < currMod.modFilesData.factionsData.factions.Count; i2++)
            {
                for (int i3 = 0; i3 < modifyDic.Count; i3++)
                {
                    ...
                    // Debug.Log(assetName);
                    if (currMod.modFilesData.factionsData.factions[i2].id == assetName)
                    {
                        contains_Bool = true;
                        // break;
    ...
    private void CheckContains(ModuleReceiver currMod)
    {
        ...
        for (int i2 = 0; i2 < currMod.modFilesData.factionsData.factions.Count; i2++)
        {
            ...
            if (contains_Bool)
            {
                ...
                for (int i3 = 0; i3 < currMod.modFilesData.factionsData.factions.Count; i3++)
                {
                    var npcCharObj = currMod.modFilesData.factionsData.factions[i3];
    ...
    void AssetAsCopy(ModuleReceiver currMod, Faction dic, string naming)
    {
        Faction data = Object.Instantiate(dic);
        ...
        if (data.factionName == "" || data.factionName == null)

        {
            data.factionName = naming;
        }
        ...
        currMod.modFilesData.factionsData.factions.Add(data);
    }

    void OverrideIfExist(ModuleReceiver currMod, Faction dic, string naming)
    {
        Faction data = Object.Instantiate(dic);
    ...
    {
        for (int index2 = 0; index2 < currMod.modFilesData.factionsData.factions.Count; index2++)
        {
            var npc = currMod.modFilesData.factionsData.factions;
            if (npc[index2].id == nameID)
            {
                ...
                currMod.modFilesData.factionsData.factions.Remove(npc[index2]);
    ...
    private void EditCurrentAssets(Faction dic, string naming)
    {
        var asstPath = AssetDatabase.GetAssetPath(dic);
    ...
    private void CreateAsset()
    {
        ...
        this.title = "Create Faction Asset";

        EditorGUILayout.HelpBox("Create Faction Asset for " + "(" + bdt_settings.currentModule + ")" + " module.", MessageType.Info);
        ...
        if (useObjectRef)
        {
            ...
            Faction eqpRef = (Faction)object_reference_A;
            object_reference_A = EditorGUILayout.ObjectField(eqpRef, typeof(Faction), true);
        ...
        assetID_new = EditorGUILayout.TextField("Faction ID:", assetID_new);
        ...
        if (contains_Bool)
        {
            ...
            for (int i3 = 0; i3 < currMod.modFilesData.factionsData.factions.Count; i3++)
            {
                var facObj = currMod.modFilesData.factionsData.factions[i3];
        ...
        Faction eqp = (Faction)ScriptableObject.CreateInstance(typeof(Faction));
        ...
        if (isSelectedRef)
        {
            if (GUILayout.Button(createAssetsLabel))
            {
                ...
                for (int i = 0; i < stack_count; i++)
                {
                    ...
                    Faction eqpAsset = ScriptableObject.CreateInstance<Faction>();

                    if (useObjectRef)
                    {
                        if (object_reference_A != null)
                        {
                            Faction npcRef = (Faction)object_reference_A;
                    ...
                    if (eqpAsset.factionName == "" || eqpAsset.factionName == null)
                    {
                        eqpAsset.factionName = assetName;
                    }
                    ...
                    path = dataPath + currMod.id + "/Factions/" + assetName + ".asset";
                    ...
                    for (int i2 = 0; i2 < currMod.modFilesData.factionsData.factions.Count; i2++)
                    {
                        if (currMod.modFilesData.factionsData.factions[i2].id == eqpAsset.id)
                        {
                            var eqpLoad = (Faction)AssetDatabase.LoadAssetAtPath(path, typeof(Faction));

                            currMod.modFilesData.factionsData.factions[i2] = eqpLoad;
                            contains_Bool = true;
                            break;
                        }
                    }
                    ...
                    if (!contains_Bool)
                    {
                        var itemLoad = (Faction)AssetDatabase.LoadAssetAtPath(path, typeof(Faction));
                        currMod.modFilesData.factionsData.factions.Add(itemLoad);
                    }
