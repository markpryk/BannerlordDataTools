...
public class SettlementsEditorManager : EditorWindow
{
    ...
    public List<Settlement> modifyDic;
    //
    ...
    private void ModifySelectedAssets()
    {
        ...
        {
            EditorGUILayout.HelpBox("Zero Settlement Selected, " + "\n" + "select at least one Settlement before edit it.", MessageType.Error);
        ...
        else
        {
            ...
            if (B_options == null)
            {
                string[] mods = Directory.GetFiles("Assets/BDT/Resources/SubModulesData", "*.asset");
            ...
            EditorGUILayout.HelpBox("Modify selected Settlement Assets.", MessageType.Info);
            ...
            if (A_index == 2)
            {
                ...
                if (GUILayout.Button("Remove Selected"))
                {
                    ...
                    foreach (var dic in modifyDic)
                    {
                        ...
                        currMod.modFilesData.settlementsData.settlements.Remove(dic);
    ...
    private void WindowModeEdit(ModuleReceiver currMod)
    {
        ...
        if (EditorGUI.EndChangeCheck())
        {
            ...
            for (int i2 = 0; i2 < currMod.modFilesData.settlementsData.settlements.Count; i2++)
            {
                for (int i3 = 0; i3 < modifyDic.Count; i3++)
                {
                    ...
                    // Debug.Log(assetName);
                    if (currMod.modFilesData.settlementsData.settlements[i2].id == assetName)
                    {
                        contains_Bool = true;
                        // break;
    ...
    private void CheckContains(ModuleReceiver currMod)
    {
        ...
        for (int i2 = 0; i2 < currMod.modFilesData.settlementsData.settlements.Count; i2++)
        {
            ...
            if (contains_Bool)
            {
                ...
                for (int i3 = 0; i3 < currMod.modFilesData.settlementsData.settlements.Count; i3++)
                {
                    var npcCharObj = currMod.modFilesData.settlementsData.settlements[i3];
    ...
    void ApplyCopyAsset(ModuleReceiver currMod)
    {
        ...
        //string modPath = "Assets/BDT/Resources/SubModulesData/" + B_options[B_index] + ".asset";
        //var destinationMod = (ModuleReceiver)AssetDatabase.LoadAssetAtPath(modPath, typeof(ModuleReceiver));
    ...
    void AssetAsCopy(ModuleReceiver currMod, Settlement dic, string naming)
    {
        Settlement data = Object.Instantiate(dic);
        ...
        if (data.settlementName == "" || data.settlementName == null)

        {
            data.settlementName = naming;
        }
       ...
       // AssetDatabase.SaveAssets();
        ...
        currMod.modFilesData.settlementsData.settlements.Add(data);
    }

    void OverrideIfExist(ModuleReceiver currMod, Settlement dic, string naming)
    {
        Settlement data = Object.Instantiate(dic);
    ...
    {
        for (int index2 = 0; index2 < currMod.modFilesData.settlementsData.settlements.Count; index2++)
        {
            var npc = currMod.modFilesData.settlementsData.settlements;
            if (npc[index2].id == nameID)
            {
                ...
                currMod.modFilesData.settlementsData.settlements.Remove(npc[index2]);
    ...
    private void EditCurrentAssets(Settlement dic, string naming)
    {
        var asstPath = AssetDatabase.GetAssetPath(dic);
    ...
    private void CreateAsset()
    {
        ...
        this.title = "Create Settlement Asset";

        EditorGUILayout.HelpBox("Create Settlement Asset for " + "(" + bdt_settings.currentModule + ")" + " module.", MessageType.Info);
        ...
        if (useObjectRef)
        {
            ...
            Settlement eqpRef = (Settlement)object_reference_A;
            object_reference_A = EditorGUILayout.ObjectField(eqpRef, typeof(Settlement), true);
        ...
        Settlement refOBJ = (Settlement)object_reference_A;
        ...
        assetID_new = EditorGUILayout.TextField("Settlement ID:", assetID_new);
        ...
        if (contains_Bool)
        {
            ...
            for (int i3 = 0; i3 < currMod.modFilesData.settlementsData.settlements.Count; i3++)
            {
                var facObj = currMod.modFilesData.settlementsData.settlements[i3];
        ...
        Settlement eqp = (Settlement)ScriptableObject.CreateInstance(typeof(Settlement));
        ...
        if (isSelectedRef)
        {
            if (GUILayout.Button(createAssetsLabel))
            {
                ...
                for (int i = 0; i < stack_count; i++)
                {
                    ...
                    Settlement eqpAsset = ScriptableObject.CreateInstance<Settlement>();

                    if (useObjectRef)
                    {
                        if (object_reference_A != null)
                        {
                            Settlement npcRef = (Settlement)object_reference_A;
                    ...
                    if (eqpAsset.settlementName == "" || eqpAsset.settlementName == null)
                    {
                        eqpAsset.settlementName = assetName;
                    }
                    ...
                    path = dataPath + currMod.id + "/Settlements/" + assetName + ".asset";
                   ...
                   // AssetDatabase.SaveAssets();
                    ...
                    for (int i2 = 0; i2 < currMod.modFilesData.settlementsData.settlements.Count; i2++)
                    {
                        if (currMod.modFilesData.settlementsData.settlements[i2].id == eqpAsset.id)
                        {
                            var eqpLoad = (Settlement)AssetDatabase.LoadAssetAtPath(path, typeof(Settlement));

                            currMod.modFilesData.settlementsData.settlements[i2] = eqpLoad;
                            contains_Bool = true;
                            break;
                        }
                    }
                    ...
                    if (!contains_Bool)
                    {
                        var itemLoad = (Settlement)AssetDatabase.LoadAssetAtPath(path, typeof(Settlement));
                        currMod.modFilesData.settlementsData.settlements.Add(itemLoad);
                    }
