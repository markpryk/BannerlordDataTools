...
public class ItemsEditorManager : EditorWindow
{
    ...
    private void ModifySelectedAssets()
    {
        ...
        {
            EditorGUILayout.HelpBox("Zero Item Selected, " + "\n" + "select at least one Item before edit it.", MessageType.Error);
        ...
        else
        {
            ...
            EditorGUILayout.HelpBox("Modify selected Item Assets.", MessageType.Info);
            ...
            if (A_index == 2)
            {
                ...
                if (GUILayout.Button("Remove Selected"))
                {
                    ...
                    foreach (var dic in modifyDic)
                    {
                        ...
                        currMod.modFilesData.itemsData.items.Remove(dic);
    ...
    private void WindowModeEdit(ModuleReceiver currMod)
    {
        ...
        if (EditorGUI.EndChangeCheck())
        {
            ...
            for (int i2 = 0; i2 < currMod.modFilesData.itemsData.items.Count; i2++)
            {
                for (int i3 = 0; i3 < modifyDic.Count; i3++)
                {
                    ...
                    // Debug.Log(assetName);
                    if (currMod.modFilesData.itemsData.items[i2].id == assetName)
                    {
                        contains_Bool = true;
                        // break;
    ...
    private void CheckContains(ModuleReceiver currMod)
    {
        ...
        for (int i2 = 0; i2 < currMod.modFilesData.itemsData.items.Count; i2++)
        {
            ...
            if (contains_Bool)
            {
                ...
                for (int i3 = 0; i3 < currMod.modFilesData.itemsData.items.Count; i3++)
                {
                    var npcCharObj = currMod.modFilesData.itemsData.items[i3];
    ...
    void AssetAsCopy(ModuleReceiver currMod, Item dic, string naming)
    {
        Item data = Object.Instantiate(dic);
        ...
        currMod.modFilesData.itemsData.items.Add(data);
    }

    void OverrideIfExist(ModuleReceiver currMod, Item dic, string naming)
    {
        Item data = Object.Instantiate(dic);
    ...
    {
        for (int index2 = 0; index2 < currMod.modFilesData.itemsData.items.Count; index2++)
        {
            var npc = currMod.modFilesData.itemsData.items;
            if (npc[index2].id == nameID)
            {
                ...
                currMod.modFilesData.itemsData.items.Remove(npc[index2]);
    ...
    private void EditCurrentAssets(Item dic, string naming)
    {
        var asstPath = AssetDatabase.GetAssetPath(dic);
    ...
    private void CreateAsset()
    {
        ...
        this.title = "Create Item Asset";

        EditorGUILayout.HelpBox("Create Item Asset for " + "(" + bdt_settings.currentModule + ")" + " module.", MessageType.Info);
        ...
        if (useObjectRef)
        {
            ...
            Item eqpRef = (Item)object_reference_A;
            object_reference_A = EditorGUILayout.ObjectField(eqpRef, typeof(Item), true);
        ...
        Item refOBJ = (Item)object_reference_A;
        ...
        assetID_new = EditorGUILayout.TextField("Item ID:", assetID_new);
        ...
        if (contains_Bool)
        {
            ...
            for (int i3 = 0; i3 < currMod.modFilesData.itemsData.items.Count; i3++)
            {
                var facObj = currMod.modFilesData.itemsData.items[i3];
        ...
        Item eqp = (Item)ScriptableObject.CreateInstance(typeof(Item));
        ...
        if (isSelectedRef)
        {
            if (GUILayout.Button(createAssetsLabel))
            {
                ...
                for (int i = 0; i < stack_count; i++)
                {
                    ...
                    Item eqpAsset = ScriptableObject.CreateInstance<Item>();

                    if (useObjectRef)
                    {
                        if (object_reference_A != null)
                        {
                            Item npcRef = (Item)object_reference_A;
                    ...
                    if (eqpAsset.itemName == "" || eqpAsset.itemName == null)
                    {
                        eqpAsset.itemName = assetName;
                    }
                    ...
                    path = dataPath + currMod.id + "/Items/" + assetName + ".asset";
                   ...
                   // AssetDatabase.SaveAssets();
                    ...
                    for (int i2 = 0; i2 < currMod.modFilesData.itemsData.items.Count; i2++)
                    {
                        if (currMod.modFilesData.itemsData.items[i2].id == eqpAsset.id)
                        {
                            var eqpLoad = (Item)AssetDatabase.LoadAssetAtPath(path, typeof(Item));

                            currMod.modFilesData.itemsData.items[i2] = eqpLoad;
                            contains_Bool = true;
                            break;
                        }
                    }
                    ...
                    if (!contains_Bool)
                    {
                        var itemLoad = (Item)AssetDatabase.LoadAssetAtPath(path, typeof(Item));
                        currMod.modFilesData.itemsData.items.Add(itemLoad);
                    }
