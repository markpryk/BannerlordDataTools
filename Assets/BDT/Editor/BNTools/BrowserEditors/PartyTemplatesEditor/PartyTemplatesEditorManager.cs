...
public class PartyTemplatesEditorManager : EditorWindow
{
    ...
    public List<PartyTemplate> modifyDic;
    //
    ...
    private void ModifySelectedAssets()
    {
        ...
        {
            EditorGUILayout.HelpBox("Zero PartyTemplate Selected, " + "\n" + "select at least one PartyTemplate before edit it.", MessageType.Error);
        ...
        else
        {
            ...
            EditorGUILayout.HelpBox("Modify selected PartyTemplate Assets.", MessageType.Info);
    ...
    void AssetAsCopy(ModuleReceiver currMod, PartyTemplate dic, string naming)
    {
        PartyTemplate data = Object.Instantiate(dic);
    ...
    private void EditCurrentAssets(PartyTemplate dic, string naming)
    {
        var asstPath = AssetDatabase.GetAssetPath(dic);
    ...
    private void CreateAsset()
    {
        ...
        this.title = "Create PartyTemplate Asset";

        EditorGUILayout.HelpBox("Create PartyTemplate Asset for " + "(" + bdt_settings.currentModule + ")" + " module.", MessageType.Info);
        ...
        if (useObjectRef)
        {
            ...
            PartyTemplate eqpRef = (PartyTemplate)object_reference_A;
            object_reference_A = EditorGUILayout.ObjectField(eqpRef, typeof(PartyTemplate), true);
        ...
        PartyTemplate refOBJ = (PartyTemplate)object_reference_A;
        ...
        assetID_new = EditorGUILayout.TextField("PartyTemplate ID:", assetID_new);
        ...
        PartyTemplate eqp = (PartyTemplate)ScriptableObject.CreateInstance(typeof(PartyTemplate));
        ...
        if (isSelectedRef)
        {
            if (GUILayout.Button(createAssetsLabel))
            {
                ...
                for (int i = 0; i < stack_count; i++)
                {
                    ...
                    PartyTemplate eqpAsset = ScriptableObject.CreateInstance<PartyTemplate>();

                    if (useObjectRef)
                    {
                        if (object_reference_A != null)
                        {
                            PartyTemplate npcRef = (PartyTemplate)object_reference_A;
                    ...
                    path = dataPath + currMod.id + "/PartyTemplates/" + assetName + ".asset";
